...
import {
  ...
  officeBelongsToMunicipalPart,
  prahaDistrictFromPsc,
  rankOfficesForPsc,
} from "../src/data/czechCityDistricts.js";
...
export function slugCityHyphen(name) {
  return String(name ?? "")
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g, "")
    .toLowerCase()
...
export function officeNameMatchesPart(officeOrName, partName) {
  return officeBelongsToMunicipalPart(officeOrName, partName);
}
...
export function officeSearchTargets(psc, units = {}) {
  ...
  return targets.sort((a, b) => {
    const rank = (target) =>
      target.type === "mestska_cast" && isNamedMunicipalPart(target.name)
        ? 0
        : target.type === "mestska_cast"
          ? 1
          : 2;
    return rank(a) - rank(b) || a.name.localeCompare(b.name, "cs");
  });
}

export function mapOfficeName(aresName, city, formCode, seatAddress = "") {
  const raw = String(aresName ?? "").trim();
  const lower = raw.toLowerCase();
  if (/městsk(á|é|ý)\s+(část|obvod)/i.test(raw)) {
    const part = stripMunicipalPrefix(raw) || city;
    const hood = parseRuianAddress(seatAddress)?.part || "";
    return `Úřad městské části ${decorateNamedDistrictLabel(part, hood)}`.trim();
  }
...
async function queryRuianLayerPost(layerId, params) {
  const body = new URLSearchParams({
    f: "json",
    returnGeometry: "false",
    ...params,
  });
  const res = await fetch(`${RUIAN_MAP}/${layerId}/query`, {
    method: "POST",
    headers: {
      Accept: "application/json",
...
export function keepAdminUnitByAddressCount(count, total) {
  const n = Number(count) || 0;
  if (n <= 0) return false;
  if (n >= 10) return true;
  return total > 0 && n / total >= 0.02;
}

async function ruianExtentForPsc(digits) {
  const data = await queryRuianLayerPost(1, {
    where: `psc=${digits}`,
...
async function countAddressesInGeometry(digits, geometry, spatialReference) {
  const data = await queryRuianLayerPost(1, {
    where: `psc=${digits}`,
    geometry: JSON.stringify(geometry),
    geometryType: "esriGeometryPolygon",
    inSR: String(spatialReference?.wkid || spatialReference?.latestWkid || 5514),
...
async function ruianUnitsIntersectingPsc(digits, extent, layerId) {
  const data = await queryRuianLayerPost(layerId, {
    geometry: JSON.stringify(extent),
    geometryType: "esriGeometryEnvelope",
    inSR: String(extent.spatialReference?.wkid || extent.spatialReference?.latestWkid || 5514),
    spatialRel: "esriSpatialRelIntersects",
    outFields: "kod,nazev",
    returnGeometry: "true",
  ...
  const counted = await Promise.all(
    features.map(async (feature) => {
      const name = String(feature?.attributes?.nazev ?? "").trim();
      const geometry = feature?.geometry;
      if (!name || !geometry) return null;
      try {
        const count = await countAddressesInGeometry(digits, geometry, extent.spatialReference);
        return { name, count };
      } catch {
        return { name, count: 0 };
      }
  ...
  const total = counted.reduce((sum, row) => sum + (row?.count || 0), 0);
  return counted
    .filter((row) => row && keepAdminUnitByAddressCount(row.count, total))
...
export async function ruianAdminUnitsForPsc(psc) {
  const digits = normalizePsc(psc);
  try {
    const extent = await ruianExtentForPsc(digits);
    if (extent?.xmin != null) {
      const [districts, obceRaw] = await Promise.all([
        ruianUnitsIntersectingPsc(digits, extent, 8),
        ruianUnitsIntersectingPsc(digits, extent, 12),
      ]);
      const obce = obceRaw.filter((row) => !isPostalCityLabel(row.name) && !isStatutoryCityName(row.name));
      if (districts.length || obce.length) {
        return {
          districts: districts.map((row) => row.name),
          obce: obce.map((row) => row.name),
          districtCounts: Object.fromEntries(districts.map((row) => [row.name, row.count])),
          obecCounts: Object.fromEntries(obce.map((row) => [row.name, row.count])),
        };
      }
    }
  } catch {
...
async function guessMunicipalWebsite(city) {
  ...
  const hyphen = slugCityHyphen(city);
  if (slug.length < 3) return null;
  const urls = [
    hyphen && hyphen !== slug ? `https://www.${hyphen}.cz` : null,
    hyphen && hyphen !== slug ? `https://${hyphen}.cz` : null,
    `https://www.${slug}.cz`,
...
function toInstitution(row, psc, website, domain) {
  ...
  const seatAddress = String(sidlo.textovaAdresa || "").trim() || null;
  const seatPsc = normalizePsc(sidlo.psc) || psc;
  return {
    id: officeIdFromIco(ico) || `inst-psc-${seatPsc}-${slugCity(city)}`,
    name: mapOfficeName(row?.obchodniJmeno, city, form, seatAddress),
    ico,
    psc: seatPsc,
    seatCity: city,
    seatAddress,
...
export async function lookupMunicipalityOfficesByPsc(psc) {
  ...
  const addRow = async (row, cityName) => {
    ...
    let domain = scraped.domain || hostnameFromWebsite(website);
    if (!domain || isPublicMailbox(domain) || !(domain.endsWith(".cz") || domain.endsWith(".eu"))) {
      domain = "";
    }
  ...
  const visibleOffices = rankOfficesForPsc(offices, digits, units);
