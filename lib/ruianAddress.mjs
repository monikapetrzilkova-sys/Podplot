...
/** Platanová 1568 | Pražská 21 | Budějovická 477/34 | Hlavní 12a */
export function splitStreetAndHouseNumber(value) {
  const raw = String(value ?? "").trim();
  if (!raw) return { street: "", houseNumber: "" };
  const match = raw.match(
    /^(.*?)[\s,]+(?:č\.?\s*(?:p\.|ev\.|popisné)?\s*)?(\d+[a-zA-Z]?(?:\/\d+[a-zA-Z]?)?)$/u
  );
...
async function searchHouses({ street, streetKod, houseNumber, city, psc }) {
  ...
  const exactNum = hnPrefix.match(/^\d+/)?.[0];
  if (
    exactNum &&
    !features.some((f) => String(f.attributes?.cislodomovni ?? "") === exactNum)
  ) {
    const extra = await queryFeatures(
      LAYER_HOUSE,
      {
        where: `ulice=${Number(kod)} AND cislodomovni=${Number(exactNum)}`,
        outFields: "kod,cislodomovni,cisloorientacni,cisloorientacnipismeno,psc,adresa,ulice",
        returnGeometry: true,
        outSR: 4326,
      },
...
export async function searchRuianAddresses({
  ...
  const split = splitStreetAndHouseNumber(st);
  const streetName = split.street || st;
  const hn = String(houseNumber ?? "").trim() || split.houseNumber;
  const wantHouses = mode === "houses" || Boolean(split.houseNumber);
  try {
    if (wantHouses) {
      return await searchHouses({ street: streetName, streetKod, houseNumber: hn, city, psc });
    }
    return await searchStreets({ street: streetName, city, psc });
  } catch {
    return { source: "ruian", items: [] };
  }
}
