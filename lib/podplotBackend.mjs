...
import { refineLocalityFromPsc } from "../src/data/czechCityDistricts.js";
import { searchRuianAddresses } from "./ruianAddress.mjs";

/** Klíč pro Maps JavaScript API v prohlížeči (smí mít HTTP referrer omezení). */
...
/**
 * Klíč pro serverové Places volání (Nearby / Details / Text Search).
 * Nesmí být omezený jen na HTTP referrery — nastavte GOOGLE_MAPS_SERVER_API_KEY
 * (Places API zapnuté, omezení: pouze API) vedle prohlížečového klíče.
 *
 * Záměrně BEZ fallbacku na getGoogleMapsApiKey(): prohlížečový klíč vrací
 * /api/config/maps komukoliv, takže by se z něj stal veřejný a neomezený
 * Places klíč účtovaný na náš účet. Když serverový klíč chybí, volající
 * spadnou na mock data (viz kontroly !getGoogleMapsServerKey() níže).
 */
...
function mapGooglePlacePhotos(photos, { max = 6, maxWidth = 900 } = {}) {
  return (photos ?? [])
    .slice(0, max)
    .map((ph) => {
      ...
      return {
        url: `/api/places/photo?${params}`,
        attribution: (ph.html_attributions ?? []).filter(Boolean).join(" ") || "Google",
      };
    })
    .filter(Boolean);
}
    ...
    openingHours:
      p.opening_hours?.open_now != null
        ? p.opening_hours.open_now
          ...
          : "Zavřeno"
        : null,
    photos: mapGooglePlacePhotos(p.photos, { max: 1, maxWidth: 640 }),
    source: "google",
  };
}
...
async function googleNearbyPage(lat, lng, radius, { type = "", keyword = "", pageToken = null } = {}) {
  ...
  url.searchParams.set("key", getGoogleMapsServerKey());
 ...
 * „fast“ = první vlna (~1 s), „vse“ = doplnění, kategorie = hlubší fetch.
 */
const GUIDE_CATEGORY_FETCH = {
  fast: {
    // Minimum typů pro okamžité špendlíky na mapě
    types: [
      "supermarket",
      "pharmacy",
      "restaurant",
  ...
  vse: {
    // Doplnění po fast — bez překryvu s fast kvůli méně duplicitním voláním
    types: [
      "doctor",
      "meal_takeaway",
      "bakery",
    ...
    textQueries: ["řeznictví", "jídelna", "pekařství", "večerka"],
    keywords: ["řeznictví"],
    pages: 1,
    deepTypes: [],
  },
  zdravi: {
    ...
    textQueries: ["zubní ordinace", "stomatologie", "lékárna", "praktický lékař"],
    pages: 2,
    deepTypes: ["dentist", "doctor"],
  },
  gastro: {
    types: ["restaurant", "meal_takeaway", "cafe", "bar", "bakery", "food"],
    textQueries: ["restaurace", "pizza", "kebab", "bistro", "kavárna", "jídelna", "vývařovna"],
    pages: 2,
  ...
  obchody: {
    ...
    keywords: ["stavebniny", "zahradnictví", "řeznictví", "maso"],
    pages: 2,
    deepTypes: ["hardware_store", "store"],
  },
  sluzby: {
...
const NEARBY_CACHE_VERSION = 13;
/** Max současných Nearby / Text Search volání (Google quota + latence). */
const PLACES_FETCH_CONCURRENCY = 12;
...
/** Názvy bez užitečného Google type (typicky jen establishment) — hřiště, řeznictví, stavebniny… */
const GUIDE_NAME_HINT_RE =
  /hřišt|hrist|dětsk[ée]\s*hři|playground|park\b|hřbitov|hrbitov|kostel|kaple|naučn|stezka|rybník|rybnik|vodopád|vodopad|\bles\b|louka|zahradnict|zahradnictví|stavebnin|hobby\s*market|\bdek\b|izomat|obi\b|bauhaus|buf[áa]č|bufac|bistro|street\s*food|sportovišt|koupališt|plovárn|řeznic|masn[áa]|uzenin|uzenář|prodej\s*masa|\bmaso\b|butcher|jídeln|vyvařov|vývařov|pekařst|pekarst|potravin|večerk|vecerk|\bradev\b/i;
...
function isUsefulGuidePlace(p, { keepSparseTypes = false } = {}) {
  if (!(p.placeId && p.name && p.lat != null && p.lng != null)) return false;
  // Hřiště/řeznictví často nemají konkrétní typ — jen establishment
  if (GUIDE_NAME_HINT_RE.test(p.name)) return true;
  if (keepSparseTypes) return true;
  const types = p.types ?? [];
...
/** Jednoduchý pool — stahuje více Nearby/Text Search najednou místo sekvence + sleep. */
...
async function googlePlacesNearby(lat, lng, radius, { type = "", category = "vse" } = {}) {
  ...
  if (type) {
    try {
      ...
      if (places.length === 0) {
        return { places: mockNearbyPlaces(lat, lng), source: "mock-fallback", category, count: 0 };
      }
  ...
  if (cached && Date.now() - cached.at < NEARBY_CACHE_TTL_MS) {
    return { places: cached.places, source: cached.source ?? "google-cache", category: cat, count: cached.places.length };
  }

  const byId = new Map();
  let denied = false;
  const addBatch = (batch, opts = {}) => {
    for (const p of batch ?? []) {
      if (p.placeId && !byId.has(p.placeId) && isUsefulGuidePlace(p, opts)) {
  ...
  const noteErr = (err) => {
    ...
    if (/referer|referrer|REQUEST_DENIED/i.test(msg)) denied = true;
    console.warn("[places]", msg);
  };

  const deep = new Set(pack.deepTypes ?? []);
  const typeJobs = (pack.types ?? []).map((t) => ({
    kind: "type",
    type: t,
    pages: deep.has(t) ? Math.max(pack.pages, 3) : pack.pages,
  }));
  const keywordJobs = (pack.keywords ?? []).map((kw) => ({
    kind: "keyword",
    keyword: kw,
    pages: Math.min(pack.pages, 2),
  }));
  const textRadius = Math.min(Math.max(Number(radius) || 2000, 1000), 50000);
  const textJobs = (pack.textQueries ?? []).map((q) => ({ kind: "text", query: q }));

  await mapPool([...typeJobs, ...keywordJobs, ...textJobs], PLACES_FETCH_CONCURRENCY, async (job) => {
    try {
      if (job.kind === "text") {
        const { places: found } = await googlePlacesTextSearch(job.query, lat, lng, textRadius);
        // Text Search často vrací jen establishment — výsledky z cíleného dotazu ponecháme
        addBatch(
          found.filter((p) => withinRadius(p, lat, lng, textRadius)),
          { keepSparseTypes: true }
        );
        return;
      }
      addBatch(
        await googleNearbyPaginated(lat, lng, radius, {
          type: job.kind === "type" ? job.type : "",
          keyword: job.kind === "keyword" ? job.keyword : "",
          maxPages: job.pages,
        }),
        { keepSparseTypes: job.kind === "keyword" }
      );
    } catch (err) {
      noteErr(err);
    }
  ...
  if (places.length === 0) {
    const mock = mockNearbyPlaces(lat, lng);
    return {
      places: mock,
      ...
      count: mock.length,
      error: denied
        ? "Serverový Google klíč je potřeba bez HTTP referrer omezení (GOOGLE_MAPS_SERVER_API_KEY)."
        : undefined,
    };
  }

  NEARBY_CACHE.set(cacheKey, { at: Date.now(), places, source: "google-cache" });
...
async function googlePlaceDetails(placeId) {
  if (placeId.startsWith("mock-google-")) {
    ...
    return mock
      ? {
          ...
          photos: [
            {
              url: "https://images.unsplash.com/photo-1554118811-1e0d58224f24?w=900&h=600&fit=crop",
              attribution: "Ukázková fotka",
            },
            {
              url: "https://images.unsplash.com/photo-1495474472287-4d71bcdd2085?w=900&h=600&fit=crop",
              attribution: "Ukázková fotka",
            },
          ],
        }
  ...
  if (!getGoogleMapsServerKey()) return null;
  const url = new URL("https://maps.googleapis.com/maps/api/place/details/json");
  url.searchParams.set("place_id", placeId);
  url.searchParams.set(
    "fields",
    "place_id,name,formatted_address,formatted_phone_number,opening_hours,rating,user_ratings_total,reviews,geometry,types,website,photos"
  );
  url.searchParams.set("key", getGoogleMapsServerKey());
  url.searchParams.set("language", "cs");
  const res = await fetch(url);
  ...
  return {
    ...
    photos: mapGooglePlacePhotos(p.photos, { max: 6, maxWidth: 900 }),
    source: "google",
  };
}

/**
 * Vyřeší photo_reference na CDN URL (bez API klíče v prohlížeči).
 * Vrací Location pro 302, nebo null.
 */
async function resolveGooglePlacePhotoUrl(photoRef, maxWidth = 800) {
  const ref = String(photoRef ?? "").trim();
  if (!ref || !getGoogleMapsServerKey()) return null;
  const width = Math.min(1600, Math.max(100, Number(maxWidth) || 800));
  ...
  if (res.ok && res.url && !res.url.includes("maps.googleapis.com/maps/api/place/photo")) {
    return res.url;
  }
  // Fallback: některé runtime automaticky followují redirect
  const followed = await fetch(url.toString(), { redirect: "follow" });
  if (followed.ok && followed.url && !followed.url.includes("key=")) {
    return followed.url;
  }
  return null;
}
...
async function proxyAddressSearch(query) {
  const photonUrl = `https://photon.komoot.io/api/?q=${encodeURIComponent(query)}&limit=12&lang=en`;
  try {
...
async function lookupPscCity(psc) {
  ...
  const city = refineLocalityFromPsc(psc, rawCity, suburb);
  ...
  return { city, psc: formatted, suburb, lat, lng };
}

const pscCenterCache = new Map();

async function handleAddressSearch({ street = "", city = "", psc = "", houseNumber = "", q = "" } = {}) {
  const st = String(street ?? "").trim();
  const cityName = String(city ?? "").trim();
  const pscDigits = String(psc ?? "").replace(/\D/g, "");
  const hn = String(houseNumber ?? "").trim();
  const fallbackQ = String(q ?? "").trim();
  const hasLocality = cityName.length >= 2 || pscDigits.length === 5;

  if (st && hasLocality) {
    let center = pscCenterCache.get(pscDigits) ?? null;
    if (!center && pscDigits.length === 5) {
      try {
        const loc = await lookupPscCity(pscDigits);
        if (loc?.lat != null && loc?.lng != null) {
          center = { lat: loc.lat, lng: loc.lng };
          pscCenterCache.set(pscDigits, center);
        }
      } catch {
        /* optional bias */
      }
    }
    const data = await searchRuianAddresses({
      street: st,
      houseNumber: hn,
      city: cityName,
      psc: pscDigits,
  ...
  const fallback = fallbackQ || [st, hn, pscDigits, cityName].filter(Boolean).join(" ").trim();
  if (fallback.replace(/\s/g, "").length < 3) {
    return { source: "empty", features: [], items: [] };
  }
  return proxyAddressSearch(fallback);
}
